...
def get_config_input():
    ...
    config["executable_path"] = input("Executable Path \n Enter just the name if it exists on path, full path otherwise: ").strip()
    ...
    config["compression_params"] = [x.strip() for x in input("Compression Parameters (comma-separated)\n For example, if algorithm uses -c for compression and -k for keeping files -c,-k can be entered: ").split(",")]
    config["decompression_params"] = [x.strip() for x in input("Decompression Parameters (comma-separated) \n For example, if algorithm uses -d for decompression and -k for keeping files -d,-k can be entered: ").split(",")]
    config["extension"] = input("Extension \n If stdout is used, extension parameter will be used as compressed file extension: ").strip()
...
def create_config_param(name, executable_path, input_file_param, output_file_param, compression_params, decompression_params, extension):
    ...
    config = {
        ...
        "output_file_param": output_file_param + "{output_file}" if output_file_param != "stdout" else "stdout",
