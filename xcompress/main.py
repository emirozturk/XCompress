...
def main():
    ...
    parser_model_compression = subparsers.add_parser("csm", help="Uses CSM (LLM) to automatically select the best compression algorithm based on your preferences (Fast compression, Fast decompression or Best compression).")
    parser_model_compression.add_argument("mode", choices=["fast-compression", "fast-decompression","best-compression"], help="The optimization mode for CSM (fast compression, fast decompression or best compression).")
    ...
    parser_brute_force.add_argument("out_folder", help="Directory where output files are stored.")
    parser_brute_force.add_argument("--delete_except_minimum", action='store_true', help="Whether to delete all compressed files except the one with the smallest size (default: False).")
    ...
    parser_benchmark.add_argument("--output_to_file", action='store_true', help="Whether to output results to a file")
    parser_benchmark.add_argument("--output_plots", action='store_true', help="Whether to generate plots from the benchmark results.")
    ...
    parser_create_config.add_argument("--input_file_param", default=None, help="The parameter for the input file in the executable command (optional).")
    parser_create_config.add_argument("--output_file_param", default=None, help="The parameter for the output file in the executable command (optional).")
    parser_create_config.add_argument("--compression_params", nargs='*', help="List of parameters for the compression process.")
    parser_create_config.add_argument("--decompression_params", nargs='*', help="List of parameters for the decompression process (if applicable).")
    ...
    parser_visualization = subparsers.add_parser("visualization", help="Visualizes benchmark results from specified JSON files.")
    parser_visualization.add_argument("file_path", help="The path to the JSON file to visualize.")
    ...
    if args.command is None:
        ...
        if args.command == "manual":
            ...
            brute_force_param(args.input_filename, args.out_folder, args.delete_except_minimum)
            ...
            for subparser in [parser_select_compression, parser_model_compression, parser_brute_force, parser_benchmark, parser_create_config, parser_visualization]:
                print("\n" + subparser.prog)
